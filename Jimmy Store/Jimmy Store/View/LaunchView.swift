...
class LaunchView: UIView {
    ...
    private let logoFrame: CGRect
    
    
    override init(frame: CGRect) {
        let size: CGFloat = 150
        let logoX: CGFloat = (frame.width / 2) - size / 2
        let logoY: CGFloat = (frame.height / 2) - size / 2
        self.logoFrame = CGRect(x: logoX, y: logoY, width: size, height: size)
        ...
        let logoPath = self.getAppleLogoPath()
...
//        animation.repeatCount = .infinity
    ...
    private func getAppleLogoPath() -> UIBezierPath {
...
extension LaunchView: CAAnimationDelegate {
    func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        if flag {
            let path = self.getAppleLogoPath()
//            t.append(UIBezierPath(rect: frame))
            
            let shapeLayer = CAShapeLayer()
            shapeLayer.path = path.cgPath
            shapeLayer.frame = self.logoFrame
            shapeLayer.fillColor = UIColor.white.cgColor
            shapeLayer.transform = CATransform3DMakeScale(1.5, 1.5, 1)
